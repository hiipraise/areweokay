...
export default function BreakupPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShareLink(`${window.location.origin}/breakup/${sessionData.sessionId}`)
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="grain-texture">
            <CardHeader>
              <CardTitle className="text-3xl flex items-center gap-2">Break Up, or Forever?<Split className="h-8 w-8 text-primary" /></CardTitle>
              <CardDescription className="text-base">Answer thought-provoking questions about your relationship to help you decide if it's time to move on or commit forever.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="block text-sm font-medium">Reflection Questions {questions.length > 0 && `(${questions.length})`}</label>
                  <Button onClick={generateQuestions} variant="outline" size="sm">{questions.length === 0 ? 'Generate Questions' : 'Regenerate'}</Button>
                </div>
                {questions.length === 0 ? (
                  <div className="p-8 bg-muted rounded-lg text-center">
                    <p className="text-muted-foreground">Click "Generate Questions" to get 10 reflective questions about your relationship</p>
                  </div>
                ) : (
              ...
              <div className="p-4 bg-primary/10 rounded-lg">
                <p className="text-sm italic text-center">"Sometimes the hardest questions lead to the clearest answers. Share these with your partner and discover your truth together."</p>
              </div>
              {shareLink ? (
